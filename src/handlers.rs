use crate::text_display_window;
use crate::{OcrWindow, selection};
use base64::Engine;
use freya::prelude::*;
use rig::{
...
pub fn handle_cursor_moved(
    cursor_pos: &mut PhysicalPosition<f64>,
...
pub fn handle_mouse_input_pressed(
    cursor_pos: &mut PhysicalPosition<f64>,
    selection: &mut Selection,
    window: &Rc<Window>,
) {
...
pub fn handle_mouse_input_released(
    windows: &mut HashMap<WindowId, OcrWindow>,
    window_id: WindowId,
    ...
    let OcrWindow { selection, .. } = windows.get_mut(&window_id).unwrap();
    ...
    if let Some((x, y, w, h)) = selection.rect() {
        ...
        if let Some(monitor) = target_monitor {
            ...
            launch(
                LaunchConfig::new().with_window(WindowConfig::new_app(TextDisplayWindow {
                    text: response.into(),
                    img_bytes,
                })),
            )
...
pub fn handle_keyboard_input(event: winit::event::KeyEvent, elwt: &EventLoopWindowTarget<()>) {
...
pub fn handle_redraw_request(windows: &mut HashMap<WindowId, OcrWindow>, window_id: WindowId) {
    let OcrWindow {
        selection,
        surface,
        window,
